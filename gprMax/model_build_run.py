...
def run_model(
    args, currentmodelrun, modelend, numbermodelruns, inputfile, usernamespace
):
    ...
    appendmodelnumber = (
        ""
        if numbermodelruns == 1 and not args.task and not args.restart
        else str(currentmodelrun)
    )
    ...
    if "G" not in globals():
        ...
        if G.messages:
            print(
                Fore.GREEN
                + "{} {}\n".format(
                    inputfilestr, "-" * (get_terminal_width() - 1 - len(inputfilestr))
                )
                + Style.RESET_ALL
            )
            ...
            if G.gpu is None:
                ...
                print(
                    "\nMemory (RAM) required: ~{} host + ~{} GPU\n".format(
                        human_size(G.memoryusage), human_size(G.memoryusage)
                    )
                )
        ...
        if all(value == 0 for value in G.pmlthickness.values()):
            ...
            if G.messages:
                if all(
                    value == G.pmlthickness["x0"] for value in G.pmlthickness.values()
                ):
                ...
                print(
                    "PML: formulation: {}, order: {}, thickness: {}".format(
                        G.pmlformulation, len(G.cfs), pmlinfo
                    )
                )
            pbar = tqdm(
                total=sum(1 for value in G.pmlthickness.values() if value > 0),
        ...
        if results["error"] and G.messages:
            ...
            raise GeneralError(
                "Non-physical wave propagation: Material '{}' has wavelength sampled by {} cells, less than required minimum for physical wave propagation. Maximum significant frequency estimated as {:g}Hz".format(
                    results["material"].ID, results["N"], results["maxfreq"]
                )
            )
        elif (
            results["deltavp"]
            and np.abs(results["deltavp"]) > G.maxnumericaldisp
            and G.messages
        ):
            print(
                Fore.RED
                + "\nWARNING: Potentially significant numerical dispersion. Estimated largest physical phase-velocity error is {:.2f}% in material '{}' whose wavelength sampled by {} cells. Maximum significant frequency estimated as {:g}Hz".format(
                    results["deltavp"],
                    results["material"].ID,
                    results["N"],
                    results["maxfreq"],
            ...
            print(
                "\nNumerical dispersion analysis: estimated largest physical phase-velocity error is {:.2f}% in material '{}' whose wavelength sampled by {} cells. Maximum significant frequency estimated as {:g}Hz".format(
                    results["deltavp"],
                    results["material"].ID,
                    results["N"],
                    results["maxfreq"],
    ...
    if G.srcsteps[0] != 0 or G.srcsteps[1] != 0 or G.srcsteps[2] != 0:
        for source in itertools.chain(G.hertziandipoles, G.magneticdipoles):
            if currentmodelrun == 1:
                if (
                    source.xcoord + G.srcsteps[0] * modelend < 0
                    or source.xcoord + G.srcsteps[0] * modelend > G.nx
                    or source.ycoord + G.srcsteps[1] * modelend < 0
                    or source.ycoord + G.srcsteps[1] * modelend > G.ny
                    or source.zcoord + G.srcsteps[2] * modelend < 0
                    or source.zcoord + G.srcsteps[2] * modelend > G.nz
                ):
                    raise GeneralError(
    ...
    if G.rxsteps[0] != 0 or G.rxsteps[1] != 0 or G.rxsteps[2] != 0:
        for receiver in G.rxs:
            if currentmodelrun == 1:
                if (
                    receiver.xcoord + G.rxsteps[0] * modelend < 0
                    or receiver.xcoord + G.rxsteps[0] * modelend > G.nx
                    or receiver.ycoord + G.rxsteps[1] * modelend < 0
                    or receiver.ycoord + G.rxsteps[1] * modelend > G.ny
                    or receiver.zcoord + G.rxsteps[2] * modelend < 0
                    or receiver.zcoord + G.rxsteps[2] * modelend > G.nz
                ):
                    raise GeneralError(
                        "Receiver(s) will be stepped to a position outside the domain."
                    )
            receiver.xcoord = (
                receiver.xcoordorigin + (currentmodelrun - 1) * G.rxsteps[0]
            )
            receiver.ycoord = (
                receiver.ycoordorigin + (currentmodelrun - 1) * G.rxsteps[1]
            )
            receiver.zcoord = (
                receiver.zcoordorigin + (currentmodelrun - 1) * G.rxsteps[2]
            )

    # Write files for any geometry views and geometry object outputs
    if (
        not (G.geometryviews or G.geometryobjectswrite)
        and args.geometry_only
        and G.messages
    ):
        print(
            Fore.RED
    ...
    if G.geometryviews:
        ...
        for i, geometryview in enumerate(G.geometryviews):
            ...
            pbar = tqdm(
                total=geometryview.datawritesize,
                unit="byte",
                unit_scale=True,
                desc="Writing geometry view file {}/{}, {}".format(
                    i + 1, len(G.geometryviews), os.path.split(geometryview.filename)[1]
                ),
                ncols=get_terminal_width() - 1,
                file=sys.stdout,
                disable=not G.progressbars,
    ...
    if G.geometryobjectswrite:
        for i, geometryobject in enumerate(G.geometryobjectswrite):
            pbar = tqdm(
                total=geometryobject.datawritesize,
                unit="byte",
                unit_scale=True,
                desc="Writing geometry object file {}/{}, {}".format(
                    i + 1,
                    len(G.geometryobjectswrite),
    ...
    # Run simulation
        ...
        # Output filename
        inputdirectory, inputfilename = os.path.split(
        ...
        if G.snapshots:
            ...
            snapshotdir = os.path.join(
                G.inputdirectory,
                os.path.splitext(G.inputfilename)[0] + "_snaps" + appendmodelnumber,
            )
            ...
            for i, snap in enumerate(G.snapshots):
                snap.filename = os.path.abspath(
                    os.path.join(snapshotdir, snap.basefilename + ".vti")
                )
                pbar = tqdm(
                    total=snap.vtkdatawritesize,
                    leave=True,
                    unit="byte",
                    unit_scale=True,
                    desc="Writing snapshot file {} of {}, {}".format(
                        i + 1, len(G.snapshots), os.path.split(snap.filename)[1]
                    ),
                    ncols=get_terminal_width() - 1,
                    file=sys.stdout,
                    disable=not G.progressbars,
        ...
        if G.messages:
            if G.gpu is None:
                ...
                print(
                    "Memory (RAM) used: ~{} host + ~{} GPU".format(
                        human_size(p.memory_info().rss), human_size(memsolve)
                    )
                )
            print(
                "Solving time [HH:MM:SS]: {}".format(datetime.timedelta(seconds=tsolve))
            )
...
def solve_cpu(currentmodelrun, modelend, G):
    ...
    for iteration in tqdm(
        range(G.iterations),
        desc="Running simulation, model " + str(currentmodelrun) + "/" + str(modelend),
        ncols=get_terminal_width() - 1,
        file=sys.stdout,
        disable=not G.progressbars,
    ):
        ...
        for source in G.transmissionlines + G.magneticdipoles:
            source.update_magnetic(
                iteration, G.updatecoeffsH, G.ID, G.Hx, G.Hy, G.Hz, G
            )
        ...
        for source in G.voltagesources + G.transmissionlines + G.hertziandipoles:
            source.update_electric(
                iteration, G.updatecoeffsE, G.ID, G.Ex, G.Ey, G.Ez, G
            )
...
def solve_gpu(currentmodelrun, modelend, G):
    ...
    if sys.platform == "win32":
        compiler_opts = ["-w"]
        ccbin = os.environ.get("GPRMAX_PYCUDA_CCBIN")
        if ccbin:
            compiler_opts.append("-ccbin={}".format(ccbin))
    else:
        compiler_opts = None
    ...
    if G.updatecoeffsE.nbytes + G.updatecoeffsH.nbytes > G.gpu.constmem:
        raise GeneralError(
            "Too many materials in the model to fit onto constant memory of size {} on {} - {} GPU".format(
                human_size(G.gpu.constmem), G.gpu.deviceID, G.gpu.name
            )
        )
    else:
    ...
    if (
        Material.maxpoles > 0
    ):  # If there are any dispersive materials (updates are split into two parts as they require present and updated electric field values).
    ...
    if G.pmls:
        # Prepare kernels
        pmlmodulelectric = (
            "gprMax.pml_updates.pml_updates_electric_" + G.pmlformulation + "_gpu"
        )
        kernelelectricfunc = getattr(
            import_module(pmlmodulelectric),
            "kernels_template_pml_electric_" + G.pmlformulation,
        )
        pmlmodulemagnetic = (
            "gprMax.pml_updates.pml_updates_magnetic_" + G.pmlformulation + "_gpu"
        )
        kernelmagneticfunc = getattr(
            import_module(pmlmodulemagnetic),
            "kernels_template_pml_magnetic_" + G.pmlformulation,
        )
        kernels_pml_electric = SourceModule(
            kernelelectricfunc.substitute(
                REAL=cudafloattype,
                N_updatecoeffsE=G.updatecoeffsE.size,
                NY_MATCOEFFS=G.updatecoeffsE.shape[1],
                NX_FIELDS=G.nx + 1,
        ...
        kernels_pml_magnetic = SourceModule(
            kernelmagneticfunc.substitute(
                REAL=cudafloattype,
                N_updatecoeffsH=G.updatecoeffsH.size,
                NY_MATCOEFFS=G.updatecoeffsH.shape[1],
                NX_FIELDS=G.nx + 1,
                NY_FIELDS=G.ny + 1,
                NZ_FIELDS=G.nz + 1,
                NX_ID=G.ID.shape[1],
    ...
    if G.rxs:
        ...
        kernel_store_outputs = SourceModule(
            kernel_template_store_outputs.substitute(
                REAL=cudafloattype,
                NY_RXCOORDS=3,
                NX_RXS=6,
                NY_RXS=G.iterations,
    ...
    if G.voltagesources + G.hertziandipoles + G.magneticdipoles:
        ...
        if G.hertziandipoles:
            srcinfo1_hertzian_gpu, srcinfo2_hertzian_gpu, srcwaves_hertzian_gpu = (
                gpu_initialise_src_arrays(G.hertziandipoles, G)
            )
            update_hertzian_dipole_gpu = kernels_sources.get_function(
                "update_hertzian_dipole"
            )
        if G.magneticdipoles:
            srcinfo1_magnetic_gpu, srcinfo2_magnetic_gpu, srcwaves_magnetic_gpu = (
                gpu_initialise_src_arrays(G.magneticdipoles, G)
            )
            update_magnetic_dipole_gpu = kernels_sources.get_function(
                "update_magnetic_dipole"
            )
        if G.voltagesources:
            srcinfo1_voltage_gpu, srcinfo2_voltage_gpu, srcwaves_voltage_gpu = (
                gpu_initialise_src_arrays(G.voltagesources, G)
            )
    ...
    if G.snapshots:
        # Initialise arrays on GPU
        snapEx_gpu, snapEy_gpu, snapEz_gpu, snapHx_gpu, snapHy_gpu, snapHz_gpu = (
            gpu_initialise_snapshot_array(G)
        )
        # Prepare kernel and get kernel function
        kernel_store_snapshot = SourceModule(
            kernel_template_store_snapshot.substitute(
                REAL=cudafloattype,
    ...
    for iteration in tqdm(
        ...
        if G.magneticdipoles:
            update_magnetic_dipole_gpu(
                ...
                G.Hz_gpu.gpudata,
                block=(1, 1, 1),
                grid=(round32(len(G.magneticdipoles)), 1, 1),
            )
        ...
        if G.hertziandipoles:
            update_hertzian_dipole_gpu(
                ...
                G.Ez_gpu.gpudata,
                block=(1, 1, 1),
                grid=(round32(len(G.hertziandipoles)), 1, 1),
            )

        # If there are any dispersive materials do 2nd part of dispersive update (it is split into two parts as it requires present and updated electric field values). Therefore it can only be completely updated after the electric field has been updated by the PML and source updates.
        if Material.maxpoles > 0:
            update_e_dispersive_B_gpu(
                np.int32(G.nx),
                np.int32(G.ny),
                np.int32(G.nz),
                np.int32(Material.maxpoles),
